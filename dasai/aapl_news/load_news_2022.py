...
# time_limits contains start-date and end-date for all months of 2022
...
def get_news_by_month(month: int):
    """
    help function to get apple news of 2022 by publishing month
    :param month: the month of the news you want
    :return: news of specified month
    """
...
# get the news for each month and concatenate
retry_counter = 0
max_retries = 3
...
while i <= 12 and retry_counter < max_retries:
    try:
        is_data_found, df_month = get_news_by_month(i)
        if is_data_found:
            # reset retry counter

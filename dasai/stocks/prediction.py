...
def predict_stock_price(
    stock_df: pd.DataFrame, output_file: str = None, train_quantile: float = 0.8
    ...
    assert all(
        column in stock_df.reset_index().columns
        for column in ["date", "adjusted_close"]
    ), (
        f"Expected columns ['date', 'adjusted_close'],"
        f" but got {stock_df.columns.tolist()} instead."
    )

    df_prophet = stock_df.reset_index().rename(
